$(function(){
       ...
       if(curPage == pageNum){
		...
		if($(this).hasClass('prevPage')){	 
			var page = curPage - 1;
			/*alert(page);*/
			window.location.href=localhost+"/Application/findBI?babyId="+babyId+"&curpage="+page;
			/*if(curPage==1){
				$('.babyList nav ul li').removeClass('active');
		...
		/*如果点击下一页*/
			...
			window.location.href=localhost+"/Application/findBI?babyId="+babyId+"&curpage="+page;
         	...
         	window.location.href=localhost+"/Application/findBI?babyId="+babyId+"&curpage="+nextPage;
         }
          /*如果下一个要点击的页码是最后一页*/
		/*if(nextPage==pageNum)
			$('.babyList nav ul li:last').addClass("disabled");*/
