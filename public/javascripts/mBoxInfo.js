$(function(){
       ...
       if(curPage == pageNum){
		...
		if($(this).hasClass('prevPage')){	 
			var page = curPage - 1;
			// alert(page);
			window.location.href = localhost+"/MedicineAction/findMBbyUser?userId="+userId+"&userName="+userName+"&curpage="+page;
		...
		/*如果点击下一页*/
			...
			window.location.href = localhost+"/MedicineAction/findMBbyUser?userId="+userId+"&userName="+userName+"&curpage="+page;
         	...
         	window.location.href = localhost+"/MedicineAction/findMBbyUser?userId="+userId+"&userName="+userName+"&curpage="+nextPage;
         }
        ...
        /*查看特定药箱里面的药物*/
	var findmdByMB = function(){

	var mbid = $(this).attr('mbid');
	var mbname = $(this).attr('mbname');
	alert(mbname+''+mbid);
	window.location.href= localhost+"/MedicineAction/findMbyMB?mbid="+mbid+"&mbname="+mbname;
	/*$.get(localhost+"/MedicineAction/findMbyMB", { mbid: 'mbid', mbname: 'mbname' } );*/
	}
    $(document).on('click','.submitBtn',findmdByMB);
